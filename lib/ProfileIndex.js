...
ProfileIndex.prototype = {
	 ...
	 */
	'generateDnCif': function( size, depth, prefix, testVar, callback ) {
		var code = "";
	...
	'generateDecodeInitFunctions': function() {
		...
		for (var i=0, l=this.index.length; i<l; ++i) {
			...
			code += this.indent + "},\n";
			code += this.indent + "init: function(inst, props) {\n";
			code += o.generateInitializer('inst','props',this.indent+this.indent, this.indent);
			code += this.indent + "}\n";
			code += "}\n\n";
		}
	...
	'generateDecodeFactory' : function(prefix) {
		...
		code += this.generateDnCif( this.index.length, 4, prefix, 'id', (function(i) {
			return "factory_"+this.index[i].safeName+";\n"
		}).bind(this));
		code += prefix + "}\n";
		return code;
	},

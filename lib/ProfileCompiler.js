...
module.exports = function( data, config, callback ) {
	...
	if (meta['uuid'] !== undefined) {
		...
		if (id > 0xFFF) {
			callback("`uuid` is a number between 0 and 0xFFF (4095)", null, null);
			return;
		}
	}
	if (meta['revision'] !== undefined) {
		...
		if (rev > 0xF) {
			callback("`revision` is a number between 0 and 0xF (15)", null, null);
			return;
		}
	}
	id = (id << 4) | rev;
	...
	// If any of the objects use embeds, we also
	// need to get a reference to the BinaryEncoder
	if (profile.hasEmbed) {
		encodeBuffer += "var BinaryEncoder = require('jbb/encoder');\n";
	}
